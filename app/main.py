...
def reg(name: str=Form(...), email: str=Form(...), password: str=Form(...)):
    d=SessionLocal()
    try:
        if d.execute(text(select(models.User).where(models.User.email==email)).scalar_one_or_none()):
...
def checkin(request: Request, file: UploadFile=File(...), lat: float=Form(None), lng: float=Form(None)):
    ...
    d=SessionLocal()
    try:
        d.execute(text('INSERT OR REPLACE INTO attendance (id,user_id,date,cin_ts,cin_lat,cin_lng,cin_photo,status) VALUES ((SELECT id FROM attendance WHERE user_id=:u AND date=:dt),:u,:dt,:ts,:lat,:lng,:ph,:st)',
